...
function oppi_register_scripts() {
    ...
    wp_register_script(
        ...
        array(), 
        '1.0.' . time(), // Usar time() para desarrollo, quitar en producción
        true
    );
    ...
    // Agregar al filtro de OpenPOS para que se cargue en el POS
    add_filter('openpos_pos_header_js', function($handles) {
        $handles[] = 'oppi-product-info';
        return $handles;
    });
    
    add_filter('openpos_pos_header_css', function($handles) {
...
add_action('wp_print_scripts', 'oppi_register_scripts');
